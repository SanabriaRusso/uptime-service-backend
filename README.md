...
4. **AWS Keyspaces Configuration**:
   ...
   - `AWS_ACCESS_KEY_ID` - Your AWS Access Key ID (same as used for S3). No need to set it if `UPTIME_SERVICE_AWS_ROLE_SESSION_NAME` and `UPTIME_SERVICE_AWS_ROLE_ARN` are set. 
   - `AWS_SECRET_ACCESS_KEY` - Your AWS Secret Access Key (same as used for S3). No need to set it if `UPTIME_SERVICE_AWS_ROLE_SESSION_NAME` and `UPTIME_SERVICE_AWS_ROLE_ARN` are set.
